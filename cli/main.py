...
def run_command(args, config):
    ...
    try:
        ...
        logger.info(
            "MCP message generated. Delivery mechanism will be implemented in future versions."
        )
...
def benchmark_command(args, config):
    ...
    for scenario_file in scenario_files:
        ...
        try:
            ...
            logger.info(
                f"Scenario {scenario_file.name} completed - Score: {evaluation['total_score']:.2f}/10"
            )

        except Exception as e:
            logger.error(f"Error processing scenario {scenario_file.name}: {str(e)}")
            results.append({"scenario": scenario_file.name, "error": str(e)})

    # Calculate overall statistics
    successful_runs = [r for r in results if "error" not in r]
    if successful_runs:
        avg_score = sum(r["evaluation"]["total_score"] for r in successful_runs) / len(
            successful_runs
        )
    ...
    for result in results:
        if "error" in result:
            ...
            print(
                f"  {result['scenario']}: {result['evaluation']['total_score']:.2f}/10"
            )
...
def main():
    ...
    create_parser.add_argument(
        "ollama_url", nargs="?", help="URL of Ollama API (default: from config)"
    )
    ...
    run_parser.add_argument(
        "--mcp-only",
        action="store_true",
        help="Only generate MCP message, don't submit proposal",
    )

    # Ask command
    ask_parser = subparsers.add_parser(
        "ask", help="Ask the AI governor a custom question about a realm"
    )
    ...
    evaluate_parser = subparsers.add_parser(
        "evaluate", help="Evaluate an AI governor's proposal quality"
    )
    ...
    evaluate_parser.add_argument(
        "--output", help="Path to save evaluation results as JSON"
    )
    evaluate_parser.add_argument(
        "--use-llm-evaluator",
        action="store_true",
    ...
    evaluate_parser.add_argument(
        "--mock", action="store_true", help="Use mock realm for testing"
    )
    evaluate_parser.add_argument(
        "--submit-mock",
        action="store_true",
    ...
    benchmark_parser = subparsers.add_parser(
        "benchmark", help="Run benchmark tests on multiple scenarios"
    )
    ...
    benchmark_parser.add_argument(
        "scenarios_dir", help="Directory containing test scenarios"
    )
    benchmark_parser.add_argument(
