...
class RealmInterface:
    ...
    def __init__(self, canister_id: str, network_url: str = "https://ic0.app"):
        ...
        if network:
            ...
            logger.debug(
                f"Using network from ASHOKA_DFX_NETWORK: {network}, network_param: {self.network_param}"
            )
        else:
            self.network_param = ""
            logger.debug("No ASHOKA_DFX_NETWORK set, using default network")

        logger.info(
            f"RealmInterface initialized for canister: {canister_id} on network: {self.network_param}"
        )
    ...
    def submit_proposal(self, proposal: Dict[str, str]) -> str:
        ...
        try:
            ...
            if not result:
                ...
                return (
                    f"Mock submission successful: {proposal['title']} (Evaluation Mode)"
                )
