...
QUALITY_INDICATORS = {
    "comprehensiveness": [
        "multiple options", "alternatives", "trade-offs", "stakeholder", 
        "perspective", "holistic", "comprehensive", "factors", "considerations"
    ],
    "practicality": [
        "implementation", "timeline", "milestones", "resources", "budget",
        "feasible", "practical", "realistic", "steps", "measurable"
    ],
    "clarity": [
        "structure", "outline", "summary", "conclusion", "introduction",
        "section", "breakdown", "clear", "concise", "objective"
    ],
    "ethics": [
        "fair", "equitable", "inclusive", "transparent", "accountable",
        "minority", "diversity", "ethical", "sustainable", "responsible"
    ]
}

def evaluate_proposal(proposal: Dict[str, str], criteria: Dict[str, float] = None) -> Dict[str, Any]:
    """
...
def _score_criterion(proposal: Dict[str, str], criterion: str) -> Tuple[float, str]:
    ...
    if criterion == "clarity":
        ...
        has_intro = bool(re.search(r"introduction|summary|overview|context", content[:len(content)//3]))
        has_conclusion = bool(re.search(r"conclusion|summary|recommendation", content[2*len(content)//3:]))
...
def _generate_summary(total_score: float, scores: Dict[str, float]) -> str:
    ...
    if total_score >= 8.5:
        return "Outstanding proposal. Well-structured, comprehensive, and implementable."
    elif total_score >= 7.0:
        return "Strong proposal. Some minor improvements possible but generally effective."
...
def evaluate_proposal_with_llm(proposal: Dict[str, str], ollama_client) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if json_start >= 0 and json_end > json_start:
            ...
            logger.warning("Could not extract JSON from LLM evaluation response, using heuristic evaluation")
