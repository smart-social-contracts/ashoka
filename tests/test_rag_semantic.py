...
def test_rag_retrieval_accuracy(rag_retriever, test_dataset):
    ...
    for question in test_questions:
        ...
        assert all("content" in ctx for ctx in contexts), "All contexts should have content"
        assert all("metadata" in ctx for ctx in contexts), "All contexts should have metadata"
...
def test_chromadb_separation(test_chromadb):
    ...
    assert test_chromadb.collection_name == "ashoka_test", "Should use test collection name"
...
def test_similarity_thresholds(embedding_pipeline, similarity_threshold):
    ...
    text2 = "DAOs must allocate treasury resources carefully with community supervision."
    ...
    if similarity_threshold <= 0.85:
        assert similarity >= similarity_threshold, f"Similar sentences should meet {similarity_threshold} threshold"
